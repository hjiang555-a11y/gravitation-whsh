...
>
> **整个实验的 Yb/Sr 值（时长加权中心值）**：**1.20750703934333772036**（R_duration = `1.2075070393433377203695609768033054790406356472332098683511270165437806214827708`）。
...
1. **整个实验的钟比值（时长加权中心值）**：R_duration = `1.20750703934333772036`，与 NIST `1.2075070393433377230(37)` 差 -2.63×10⁻¹⁸，与实验方 WLS `1.2075070393433377213(23)` 差 -0.93×10⁻¹⁸。逐段 y_i ∈ [-4.6, 6.4]×10⁻¹⁸。
...
| 维度 | 结果 |
|---|---|
| 整个实验 Yb/Sr 值 | R_duration = 1.20750703934333772036，与实验方 WLS 差 -0.93×10⁻¹⁸ |
...
|---|---|---|---|
| WLS（1/u_i²） | 1.20750703934333772079 | 2.753e-19 | -3.159e-19 |
| Birge（B=2.324，1/u_i²） | 1.20750703934333772079 | 6.399e-19 | -3.159e-19 |
| Mandel-Paule（ξ=2.465e-18，1/(u_i²+ξ²)） | 1.20750703934333772128 | 7.551e-19 | -4.408e-19 |
| 贝叶斯（ξ=2.613e-18） | 1.20750703934333772128 | 8.004e-19 | -4.433e-19 |

WLS 拟合度：χ² = 86.42（dof=16，χ²_red = 5.40，p = 1.14e-11）。

> **口径说明（独立验证后）**：本套每段统计不确定度 u_i 由 OADEV 外推得到（拍频归一化到 1550 nm 光频，128s≤τ≤T/4 拟合斜率外推到 Teff）。独立核对发现：拍频数据**不是纯白频率噪声**（OADEV 曲线在 τ≳4096 s 后出现低频平台，偏离 τ^(-1/2)），故外推得到的 σ_y(Teff) 对短段偏高、长段偏低，u_i 分布极分散（极差约 18 倍）。这导致本套 WLS 的 χ²_red≈5.4、Birge ratio≈2.3，高于实验方 PDF 的 χ²_red=1.70、Birge=1.30。差异来源是 OADEV 外推对非白噪声数据的系统性偏差，**不是潮汐修正**（潮汐是日周期、半日周期信号，其贡献在 τ≲1 h 的 OADEV 段应表现为额外低频结构，而非本套所有 u_i 的尺度问题）。此并列结果的 u_i 与四种合并值需在获得正确的 OADEV 外推（如改用 MDEV 或限制外推区间）后重算，当前仅作方法演示。
