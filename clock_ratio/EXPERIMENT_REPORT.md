...
理论（A=−1）补偿令 χ²_red 从 5.424 降到 3.699（−31.8%），经验（A=−0.54）降到 4.559（−15.9%）。用两个口径衡量其显著性：

**口径一：似然比 Δχ²（把「加回潮汐」当作 1 参数模型扩展）**。
theory 的 χ² 从 86.78 降到 59.19，Δχ² = 27.59（dof=1），p = 1.50e-07（等效 5.3σ）；
empirical 的 χ² 从 86.78 降到 72.95，Δχ² = 13.83（dof=1），p = 2.00e-04（等效 3.7σ）。
即潮汐补偿对拟合的改善**高度显著**（远过 5σ / 3.7σ）。

**口径二：补偿本身并非全解**。三个情景自己的 χ² 拟合优度 p 值仍都 ≪ 0.05
（raw 9.8e-12、theory 7.2e-07、empirical 3.0e-09），组间**超额散布**等效显著度从 raw 的 6.8σ 降到 theory 的 5.0σ。
也就是说：理论补偿**显著改善**了组间一致性，但**没有消除全部**超额散布——
残余 χ²_red≈3.7 意味着除潮汐外仍有未建模的组间随机过程。这一点与 §6 的
u_i 不可靠结论一致，故改善的显著性不等同于「已完全解释组间散布」。

---

## 8. 段 9 剔除敏感性检查（附加结果，不替代 §7）

段 9 的 Sr 系统频移 `shift_a = −8.13×10⁻¹⁷` 介于 7 月值（≈−1.72×10⁻¹⁶）
与 8 月值（≈+7.9×10⁻¹⁸）之间，源码自注「请确认是真实值还是占位符」
（[METHODOLOGY §8](../docs/METHODOLOGY.md)），使其 y_9 偏大、成为原始组间散布的主
导离群点。本节把段 9 剔除后**重新合并**已有的逐段 (y_i, u_i)，回答「第 9 组若提出，
结果变化多少」。**这是附加敏感性检查，17 段主结果（§7）原样保留、不被替换。**

四种统计方法（WLS / Birge / Mandel–Paule / 贝叶斯）的合并中心值（17 段 → 剔除段 9 的 16 段）：

| 情景 | A | WLS R | M-P R（ξ） | 贝叶斯 R（ξ） |
|---|---|---|---|---|
| raw | 0 | 1.20750703934333772079→1.20750703934333772014 | 1.20750703934333772128→1.20750703934333772058（2.47e-18→1.84e-18） | 1.20750703934333772128→1.20750703934333772037（2.61e-18→9.80e-19） |
| theory | -1 | 1.20750703934333772168→1.20750703934333772110 | 1.20750703934333772185→1.20750703934333772144（2.10e-18→1.89e-18） | 1.20750703934333772184→1.20750703934333772132（2.13e-18→1.32e-18） |
| empirical | -0.54 | 1.20750703934333772125→1.20750703934333772062 | 1.20750703934333772158→1.20750703934333772104（2.18e-18→1.79e-18） | 1.20750703934333772157→1.20750703934333772096（2.29e-18→1.52e-18） |

| 情景 | A | χ²_red | Birge B | u_WLS |
|---|---|---|---|---|
| raw | 0 | 5.424→2.684 | 2.329→1.638 | 2.748e-19→2.858e-19 |
| theory | -1 | 3.699→2.821 | 1.923→1.680 | 3.345e-19→3.547e-19 |
| empirical | -0.54 | 4.559→2.919 | 2.135→1.709 | 3.046e-19→3.199e-19 |

> **关键变化**：剔除段 9 后，**潮汐补偿「降低 χ²_red」的效应大幅减弱甚至反号**，
> 且**四种方法一致**：
> 17 段时 theory 相对 raw 的 χ²_red 降幅为 +31.8%，剔除段 9 后 raw 的 χ²_red
> 从 5.424 骤降到 2.684（最大离群点被移除），
> 而 theory 只从 3.699 降到 2.821，两者差距缩小到
> 同量级——即 §7.3「补偿显著改善组间一致性」的证据**高度依赖段 9 这个离群点**。
> WLS / Mandel–Paule / 贝叶斯三种中心值在剔除段 9 后都整体下移约 −0.6×10⁻¹⁸，
> 最接近实验方 WLS 的情景由 17 段的 empirical 变为 16 段的 theory（|偏差| 0.20×10⁻¹⁸）。
> 这提示：**段 9 的 shift_a 异常是组间散布的主要来源之一**，其真实性未定前，
> §7 的 χ²_red 改善与长期稳定度结论都应谨慎解读。数值来源
> [statistical_methods_tidal_seg9_excluded.json](statistical_methods_tidal_seg9_excluded.json)。
