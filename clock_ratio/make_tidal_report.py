...
- **方向自检**：用「修正后潮汐相关性是否消失」自检 −0.54 的方向与幅度——沿历史 `batch_analysis.py` 同一 1200-s 三角窗去均值处理链，对每段构造 `resid = beat − A_fix·tide` 再拟合，A_fix=−0.54 时残差段均值 r 从 −0.134 塌到 +0.003、带符号 Stouffer z 从 −5.16 掉到 +0.39（p=0.69）、负相关段 8/17 随机（p=1.000）；网格扫描的残差相关性零点恰为 A_fix≈−0.54。这确认 −0.54 的符号方向与幅度都补对了，不依赖尚未定死的 s_beat 梳尺/本振符号链。但此自检针对去均值波形，不含 DC；且 |z|=4.33、Fisher p=0.086 仍略显著，说明残差可能残留少量确定结构，不否定「方向正确」。
