...
def main() -> int:
    ...
    if stat is not None:
        ...
        L.append("")
        L.append("> **口径说明（独立验证后）**：本套每段统计不确定度 u_i 由 OADEV 外推"
                 "得到（拍频归一化到 1550 nm 光频，128s≤τ≤T/4 拟合斜率外推到 Teff）。"
                 "独立核对发现：拍频数据**不是纯白频率噪声**（OADEV 曲线在 τ≳4096 s 后"
                 "出现低频平台，偏离 τ^(-1/2)），故外推得到的 σ_y(Teff) 对短段偏高、"
                 "长段偏低，u_i 分布极分散（极差约 18 倍）。这导致本套 WLS 的 χ²_red≈5.4、"
                 "Birge ratio≈2.3，高于实验方 PDF 的 χ²_red=1.70、Birge=1.30。"
                 "差异来源是 OADEV 外推对非白噪声数据的系统性偏差，**不是潮汐修正**"
                 "（潮汐是日周期、半日周期信号，其贡献在 τ≲1 h 的 OADEV 段应表现为"
                 "额外低频结构，而非本套所有 u_i 的尺度问题）。此并列结果的 u_i 与"
                 "四种合并值需在获得正确的 OADEV 外推（如改用 MDEV 或限制外推区间）"
                 "后重算，当前仅作方法演示。")
        L.append("")
