...
- 日平均无漂移（日平均 std=0.011 m²/s²，远小于时变 std=0.371）
...
潮汐势差与实测 1550 nm 环外链路拍频（FXE_B8）及 17 组无跳点实验记录比对，完整
报告见 [clock/ANALYSIS.md](clock/ANALYSIS.md)。

**结论摘要**（专业综合差数据，17 段全部有数据）：

- **17 段无跳点批量分析**：17 段全部有数据（第 15/16/17 段为 8 月 21–26 日新增）；
  单段幅度拟合大多不显著（潮汐信号 Δf/f rms ~4.8e-18 被链路噪声 ~1.7e-17 淹没约
  3.6 倍），但 **14/17 段相关系数为负，跨段合并后显著负相关**（符号无关 Stouffer
  |z| = 5.91，p = 3.4e-9，加权 r ≈ −0.13，A = −0.46±0.07/6.5σ）。
- **关键发现**：这一致负向最可能是潮汐数据符号方向相反；若方向取反（`−ΔW/c²`），
  潮汐引力红移以正确方向、部分幅度被检出。确认需 Yb/Sr 钟部署站点与拍频符号
  约定。
- 会话均值相关性 Pearson r = +0.468（p = 0.091），方向为正、较旧数据提升但仍不
  显著（y_i = R_i/R_ref − 1 由 MATLAB 处理程序精确计算；暂 14 段，17 段逐段 y_i
  待实验方导出）。
...
主计算链新增 `--professional-tidal` 入口，直接以专业 30 秒综合差为权威总势差。
