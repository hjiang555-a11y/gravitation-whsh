...
- 潮汐引力红移在无跳点段拍频数据中**不可检出**。第 13 组（42.5 h，0 跳点）
  幅度拟合 `A = −0.39 ± 0.23`（离理论 +1 约 6.1σ）；第 6 组（10 h 可用，0 跳点）
  `A = −0.26 ± 0.50`。
- 信号（Δf/f rms ~4.8e-18）被 1550 nm 链路噪声（积分后仍 ~1.7e-17）淹没约一个
  量级。
- 14 组会话均值相关性 Pearson r = +0.355（p = 0.213），方向为正但不显著。
- 存在未决的符号问题（Yb/Sr 钟部署站点未明确），当前判定以「假设潮汐模板为
  +ΔW/c² 时不可检出」为准。

**已修正的两处错误**：拍频时间戳为北京时间（UTC+8）需 `−8 h` 对齐 UTC 潮汐 CSV；
