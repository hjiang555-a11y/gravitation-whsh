...
`δW = −γ·δh`（γ 为 Somigliana 正常重力）转换为随形势变化。
...
| 海潮负荷势差 | −0.138 | +0.164 | 0.302 | 0.060 |
| **总潮汐势差** | **−0.763** | **+0.673** | **1.437** | **0.284** |

（单位：m²/s² = J/kg）

- 总潮汐势差峰值出现在 2026-07-14T07:29 UTC，达 **−0.763 m²/s²**
- 相当于质量为 m 的物体势能变化 `m × ΔW` J；每千克能量变化范围 **−0.763 ~ +0.673 J/kg**
- 海潮负荷贡献约占总信号的 ~21%（std 比 0.060/0.284），不可忽略
...
- **14 段无跳点批量分析**（9 段有数据，组 1–5 缺 7 月数据文件）：单段幅度拟合
  均不显著（潮汐信号 Δf/f rms ~4.8e-18 被链路噪声 ~1.7e-17 淹没约 3.6 倍），但
  **8/9 段相关系数为负，跨段合并后显著负相关**（Stouffer z = −4.2，p < 0.001，
  加权 r ≈ −0.16）。
- **关键发现**：这一致负向最可能是潮汐模板符号方向相反；若方向取反（`−ΔW/c²`），
  潮汐引力红移以正确方向、部分幅度被检出。确认需 Yb/Sr 钟部署站点与拍频符号
  约定。
- 14 组会话均值相关性 Pearson r = +0.355（p = 0.213），方向为正但不显著（y_i 为
  PDF 散点图数字化近似值）。

**已修正的四处错误**：拍频时间戳为北京时间（UTC+8）需 `−8 h` 对齐 UTC 潮汐 CSV；
