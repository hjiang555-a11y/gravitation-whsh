...
- [ ] **Love 数 k₂ 未分阶**：统一用 0.30190，应改为 m=0→0.30190 / m=1→0.29830 / m=2→0.30102。
- [ ] **缺频率相关 δk 改正**（表 6.5a/b/c）。
- [ ] **星历用第三方 DE421** → 换官方 de440s.bsp（SHA-256 锚定）。

### P2 — 文档/代码一致性

- [ ] 缺 `.gitignore`（`.pyc`、`egg-info` 入库）。
- [ ] README「44 分潮」与文件头「34 分潮」表述混淆。
- [ ] HARPOS 静默覆盖 BLQ 未文档化。
- [ ] BLQ 路径角色（备用）未说明。

## 4. 实施阶段

1. **阶段 1（正确性修复）**：修 SHAO 坐标 + README 同步。✅ 已完成
2. **阶段 2（可信度升级）**：de440 星历 + IERS2010 Step1+Step2 固体潮。
3. **阶段 3（验证）**：复现 DEHANTTIDEINEL 基准 + pyTMD/ETGTAB 交叉验证 + 武汉半日潮基准。
4. **阶段 4（文档对齐）**：`.gitignore` + README 数据溯源与优先级说明。
