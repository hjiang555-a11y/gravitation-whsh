...
- [x] **缺频率相关 δk 改正**（表 6.4/6.5c/7.1 + 式 6.12/7.4a，Step 2）→ 已实现。实测对有效势因子 (1+k−h) 的影响：K1 日潮 +6.1%（FCN 共振，k₂ 偏离名义值 −13.2%）、P1 +2.1%、长周期 −1%，CT 振幅加权 ~1.35%；对输出量 SHAO−WUHN 差 rms=0.0050 m²/s²（~0.5% 相对）。
...
3. **阶段 3（验证）**：pyTMD 交叉验证 ✅（见 §5）；DEHANTTIDEINEL 位移基准 ✅（见 §6）；武汉 2000 半日潮重力基准 ✅（见 §7）。**全部完成**。
...
重力潮汐因子 δ₂ = 1 + h₂ − (3/2)k₂ 将观测重力潮汐与潮汐生成势关联，同时用到
位移 Love 数 h₂ 与势 Love 数 k₂。用本项目实现的 IERS 2010 Love 数（Step 1 阶相关
+ Step 2 频率相关）计算四个主潮波的理论 δ，与武汉超导重力仪基准值比对：

| 潮波 | 理论 δ（本项目） | 武汉 2000（含海潮负荷） | 2014 精化（负荷改正后） |
|---|---:|---:|---:|
| O1 | 1.15638 | 1.1780 | 1.15841 |
| K1 | 1.13786 | 1.1522 | — |
| M2 | 1.15627 | 1.1751 | 1.16410 |
| S2 | 1.15627 | 1.1710 | — |

**结论**：半日潮带（M2/S2）频率无关，δ 相同；日潮带（O1/K1）受 FCN 共振影响呈强
频率相关。理论 M2 与 2014 负荷改正后观测值差距 **+0.68%**，这正是 IERS 2010 名义
Love 数与 DDW99 非流体静力平衡滞弹性模型（Dehant-Defraigne-Wahr 1999）的**已知
差距**，与 2014 论文（14 年 SG C032 精化）明确指出的"M2001 弹性模型偏差达 0.67%"
一致——是本模型的**精度上限说明**，非实现误差。

**源数据发现**（诚实记录）：
- 武汉 2000 基准值（Xu et al. 2000, *Sci. China D* 43(1):77–83）**含海潮负荷**，
  不能直接与理论固体潮 δ 比对；需用 2014 负荷改正后的精化值（M2=1.16410, O1=1.15841）。
- 2014 论文明确：武汉基准在半日潮带"非常准确（偏差 <0.07%）"，日潮带"需精化"，
  故本比对以半日潮 M2 为主要判据、O1 为参考。

可复现脚本：[validation/wuhan_reference_benchmark.py](validation/wuhan_reference_benchmark.py)。
