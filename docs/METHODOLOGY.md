...
### 9.5 方向自检：−0.54 修正后潮汐相关性消失

本仓用一种与符号链无关的方式自检修正方向：**若 −0.54 补对了，修正后的拍频残差
应与潮汐模板不再系统相关**。做法是沿历史 `batch_analysis.py` 完全相同的处理链
（1200-s 三角窗、去均值、`beat=A·tide+noise`），对每段用固定 `A_fix` 构造残差
`resid = beat − A_fix·tide`，再拟合残差对同一个潮汐模板的幅度与相关性：

| 量 | 修正前（raw） | 修正后（A_fix=−0.54） |
|---|---|---|
| 段均值幅度 A（未加权） | −0.340 | +0.200 |
| 段均值相关系数 r | −0.134 | **+0.0027** |
| 负相关段数 | 14/17（二项 p=0.013） | 8/17（二项 **p=1.000**） |
| Stouffer（带符号）z | −5.16 | **+0.39（p=0.69）** |
| Fisher 合并 p | 2.2×10⁻⁶ | **0.086** |

修正后 r 均值塌到 ≈0、带符号 Stouffer z 掉到 +0.39（不显著）、负相关段 8/17
完全随机。网格扫描 `A_fix ∈ {−1.0, −0.54, −0.30, −0.20, 0, +0.20, +0.54, +1.0}`
得到的残差相关性零点恰好位于 **A_fix ≈ −0.54**，与历史精度加权幅度
A=−0.5397±0.084 一致。这从「修正后相关性消失」直接确认 −0.54 的**符号方向与幅度**都正确，
而不必依赖尚未定死的 s_beat 梳尺/本振符号链。

> 注意两点边界：其一，此自检用的是**去均值波形**（历史拟合无截距），故 DC 部分
> 不参与该相关性检验，不能据此宣称均值（潮汐的 DC）也已被校准——那仍是 §9.1 的
> 额外假设。其二，A_fix=−0.54 对 p_i 作组合时，符号无关的 |z|=4.33 仍略显著、
> Fisher p=0.086，反映残差里可能残留少量确定结构（如段 14 的单段 r=−0.21，
> p=0.02），但已无系统性方向一致的相关性。这三点不改变「方向正确」的结论。
...
把 §10.2 逐情景的合并值跨方法、跨情景凝练成**总效应**（报告 §7.1 的来源），
以实验方 WLS 参考 `1.2075070393433377213` 为横尺：

| 方法 | raw（A=0） | theory（A=−1） | empirical（A=−0.54） | 偏差单位 |
|---|---|---|---|---|
| R_wls 偏差 | −0.503 | +0.389 | **−0.041** | ×10⁻¹⁸ |
| R_mp 偏差 | −0.013 | +0.556 | +0.283 | ×10⁻¹⁸ |
| R_bayes 偏差 | −0.016 | +0.549 | +0.278 | ×10⁻¹⁸ |

**总效应**：修正把整个实验的 WLS 中心从 raw 的 −0.50×10⁻¹⁸ 移到 empirical 的
−0.04×10⁻¹⁸（最接近实验方 WLS），theory 反号到 +0.39×10⁻¹⁸；χ²_red 从 5.42 单调
降到 theory 的 3.70（empirical 4.56），Birge 比与 M-P/贝叶斯 ξ 同步减小——「加回
潮汐」显著降低了组间散布。但 §3 的 u_i 不可靠结论（OADEV 外推系统低估 + 个别段
u_i 异常）在此同样成立，合并值仅供方法比较，`u` 不是最终不确定度；方向自检见 §9.5。

### 10.4 长期稳定度改善（跨段钟比值散布，报告 §7.2）

潮汐是周期约 12/24 h 的慢信号，主要影响**跨段（天级）长期散布**，而非单段内部
短时 OADEV（见 §9.4/§9.5）。用 17 段钟比值偏差 `y_i,scenario`（相对同情景段 1，×10⁻¹⁸）
的**段间样本标准差**作为长期稳定度度量：

| 情景 | 段间 σ(y_i)（×10⁻¹⁸） | 相对 raw 改善 |
|---|---|---|
| raw（A=0） | 2.980 | 基准 |
| theory（A=−1） | 2.588 | **+13.2%** |
| empirical（A=−0.54） | 2.596 | **+12.9%** |

**结论**：补偿潮汐后钟比对长期稳定度**有改善**——段间钟比值散布从 2.98×10⁻¹⁸ 降到
约 2.59×10⁻¹⁸（改善 ~13%）；组间方差指标 χ²_red 降 16–32%、Birge 比降 8–17%（§10.2），
方向一致。theory（完全信任潮汐理论）改善最大、empirical（经验幅度）次之。这仍是
方法比较层面的描述性结论，不改变 §3 的 u_i 不可靠、不作最终不确定度声明的口径。

### 10.5 χ²_red 下降的显著性（报告 §7.3）

理论（A=−1）补偿令 χ²_red 从 5.42 降到 3.70（−31.8%），经验（A=−0.54）降到 4.56
（−15.9%）。用两个口径衡量该改善是否显著：

1. **似然比 Δχ²（把「加回潮汐」当 1 参数模型扩展）**：theory 的 χ² 从 86.78 降到
   59.19，`Δχ² = 27.59`（dof=1），`p = 1.5e-7`，等效 **5.3σ**；empirical 的 χ² 从
   86.78 降到 72.95，`Δχ² = 13.83`（dof=1），`p = 2.0e-4`，等效 **3.7σ**。即潮汐补偿对
   拟合的改善**高度显著**。
2. **补偿并非全解**：三个情景自己的 χ² 拟合优度 p 都仍 ≪ 0.05（raw 9.8e-12、theory
   7.2e-7、empirical 3.0e-9），组间超额散布等效显著度从 raw 的 6.8σ 降到 theory 的
   5.0σ。即理论补偿**显著改善**了组间一致性，但**没有消除全部**超额散布——残余
   χ²_red≈3.7 说明除潮汐外仍有未建模的组间随机过程，与 §3 的 u_i 不可靠结论一致。

> **口径边界**：A 是固定系数（非本次拟合的自由参数），故「Δχ²、dof=1」的似然比检验
> 是把「加回潮汐」当作单参数模型扩展的**描述性**度量，不是严格的嵌套模型检验；其
> 5.3σ/3.7σ 应理解为「补偿方向与幅度带来的拟合改善的显著程度」，而非潮汐参数本身的
> 拟合置信。真正的显著性检验需在 u_i 可靠（§3 待解决）之后重做。

---

## 11. 段 9 剔除敏感性检查（`statistical_methods_tidal_seg9.py`，附加）

段 9 的 `shift_a = −8.13×10⁻¹⁷` 异常（§8.2/§8.3）使其 `y_9` 偏大、主导原始组间
散布。本节把段 9 剔除后**重新合并** `statistical_methods_tidal.json` 中已有的逐段
`(y_i, u_i)`，回答「第 9 组若提出，结果变化多少」。实现见
[statistical_methods_tidal_seg9.py](../clock_ratio/statistical_methods_tidal_seg9.py)，
产物 [statistical_methods_tidal_seg9_excluded.json](../clock_ratio/statistical_methods_tidal_seg9_excluded.json)；
报告 §8。**这是附加敏感性检查，17 段主结果（§10、报告 §7）原样保留、不被替换。**

- 方法：读源 JSON 的逐段行，丢掉 `group==9` 的 1 行（17→16），其余 (y_i, u_i) 原样
  保留，沿用同情景 `R_seg1` 与同一 Decimal 重建路径再跑 WLS/Birge/M-P/贝叶斯。
  不重新做样本选择、不重新做潮汐修正。
- 结果：

| 情景 | χ²_red（17段→16段） | Birge（17段→16段） |
|---|---|---|
| raw | 5.424 → 2.684 | 2.329 → 1.638 |
| theory | 3.699 → 2.821 | 1.923 → 1.680 |
| empirical | 4.559 → 2.919 | 2.135 → 1.709 |

- **关键结论**：剔除段 9 后，**潮汐补偿「降低 χ²_red」的效应大幅减弱甚至反号**。
  17 段时 theory 相对 raw 的 χ²_red 降幅为 +31.8%（即 theory 更好 31.8%），剔除段 9 后
  raw 的 χ²_red 骤降 5.42→2.68（最大离群点被移除），而 theory 只从 3.70 降到 2.82，
  theory 相对 raw 变为 −5.1%（略微更差），empirical 为 −8.8%。三情景 WLS 中心值整体
  下移约 −0.6×10⁻¹⁸，最接近实验方 WLS 的情景由 17 段的 empirical 变为 16 段的 theory
  （|偏差| 0.20×10⁻¹⁸）。
- **解读边界**：段 9 的 `shift_a` 真实性未定，其异常是组间散布的主要来源之一；因此
  §10 的 χ²_red 改善与长期稳定度结论**高度依赖段 9**，需谨慎解读，直到段 9 的
  `shift_a` 被确认。两者都不改变 §3 的 u_i 不可靠口径。
