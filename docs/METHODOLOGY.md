...
> ⚠️ **已知存疑（已独立验证）**：拍频数据**不是纯白频率噪声**——OADEV 曲线在
> τ ≳ 4096 s 后出现低频平台，偏离 τ^(−1/2)。因此外推得到的 σ_y(T) 对短段偏高、
> 长段偏低，u_i 分布极分散（极差约 18 倍）。这导致本套 WLS 的 χ²_red ≈ 5.4、Birge
> ratio ≈ 2.3，高于实验方 PDF（χ²_red=1.70、Birge=1.30）。差异来源是 OADEV 外推
> 对非白噪声数据的系统性偏差，**不是潮汐修正**。详见报告第 6 章口径说明。
...
1. **OADEV 外推对非白噪声数据的系统性偏差**（§2.2）——导致 χ²_red≈5.4、Birge≈2.3，与实验方不同。需改用 MDEV 或限制外推区间后重算。
