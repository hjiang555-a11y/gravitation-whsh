...
- 方法：读源 JSON 的逐段行，丢掉 `group==9` 的 1 行（17→16），其余 (y_i, u_i) 原样
  保留，沿用同情景 `R_seg1` 与同一 Decimal 重建路径再跑 WLS/Birge/M-P/贝叶斯四种方法。
  不重新做样本选择、不重新做潮汐修正。
- 结果：四种统计方法的合并中心值（17 段 → 16 段）：

| 情景 | WLS R | M-P R（ξ） | 贝叶斯 R（ξ） |
|---|---|---|---|
| raw | …72079→…72014 | …72128→…72058（2.47e-18→1.84e-18） | …72128→…72037（2.61e-18→9.80e-19） |
| theory | …72168→…72110 | …72185→…72144（2.10e-18→1.89e-18） | …72184→…72132（2.13e-18→1.32e-18） |
| empirical | …72125→…72062 | …72158→…72104（2.18e-18→1.79e-18） | …72157→…72096（2.29e-18→1.52e-18） |

| 情景 | χ²_red（17段→16段） | Birge B（17段→16段） | u_WLS（17段→16段） |
|---|---|---|---|
| raw | 5.424→2.684 | 2.329→1.638 | 2.748e-19→2.858e-19 |
| theory | 3.699→2.821 | 1.923→1.680 | 3.345e-19→3.547e-19 |
| empirical | 4.559→2.919 | 2.135→1.709 | 3.046e-19→3.199e-19 |

- **关键结论**：剔除段 9 后，**潮汐补偿「降低 χ²_red」的效应大幅减弱甚至反号，且四种方法一致**。
  ...
  theory 相对 raw 变为 −5.1%（略微更差），empirical 为 −8.8%。WLS / Mandel–Paule /
  贝叶斯三种中心值整体下移约 −0.6×10⁻¹⁸，最接近实验方 WLS 的情景由 17 段的 empirical
  变为 16 段的 theory（|偏差| 0.20×10⁻¹⁸）。
