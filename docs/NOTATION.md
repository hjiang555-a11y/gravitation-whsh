...
**方向自检**：用「修正后潮汐相关性消失」而非 s_beat 符号链自检 −0.54 的方向与幅度。
沿历史 [batch_analysis.py](../clock/segment_analysis/batch_analysis.py) 同一 1200-s 三角窗
去均值处理链，对每段构造 `resid = beat − A_fix·tide` 再拟合；A_fix=−0.54 时残差段均值
r 从 −0.134 塌到 +0.003、带符号 Stouffer z 从 −5.16 掉到 +0.39（p=0.69）、负相关段 8/17
随机（p=1.000）；网格扫描的残差相关性零点恰为 A_fix≈−0.54。据此 −0.54 的符号方向与幅度
都补对了，不依赖尚未定死的 `s_beat`。此自检只覆盖去均值波形（不含 DC），且残差符号无关
|z| 与 Fisher p 仍略显著（残留少量确定结构），不宣称「相关完全消失」——详见
[METHODOLOGY §9.5](METHODOLOGY.md#95-方向自检−054-修正后潮汐相关性消失)。
