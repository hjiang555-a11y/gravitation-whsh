...
> **整个实验的 Yb/Sr 值（本库重建）**：**1.207 507 039 343 337 7204**
> （= 17 段按有效时长加权的中心值 R_wls = `1.20750703934333772036956097…`，
...
| `R_ref` | **第 1 段**的钟比值 R_1，仅作 y_i 的相对基准（**不是**整个实验值）|
| `R_wls` | **整个实验的 Yb/Sr 值** = 17 段按有效时长加权的中心值 `Σ(n_valid_i·R_i)/Σn_valid_i` |
| `y_i` | 段钟比值偏差 = `R_i/R_ref − 1`（×10⁻¹⁸）|
...
> **关键区分**：`R_ref`（段1）与 `R_wls`（整个实验加权值）是**两个不同量**，
> 不可混淆——前者只是 y_i 的相对基准，后者才是整个实验的 Yb/Sr 值。
...
1. **钟比值（整个实验值）**：R_wls = `1.207507039343337720370…`（17 段时长加权
   中心值），与 NIST 参考 `1.2075070393433377230(37)` 差 −2.63×10⁻¹⁸，与实验方
   WLS `1.2075070393433377213(23)` 差 −0.93×10⁻¹⁸。（注：R_ref = 段 1 的值，仅作
   y_i 基准，非整个实验值。）
