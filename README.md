...
- **方向自检**：用「修正后潮汐相关性消失」而非 s_beat 符号链确证 −0.54 的方向与幅度。
  沿历史 1200-s 三角窗去均值处理链构造 `resid = beat − A_fix·tide` 再拟合；A_fix=−0.54 时
  残差段均值 r 从 −0.134 塌到 +0.003、带符号 Stouffer z 从 −5.16 掉到 +0.39（p=0.69）、
  负相关段 8/17 随机；网格扫描的残差相关性零点恰为 A_fix≈−0.54。据此补对方向，不依赖
  未定死的 s_beat。此自检只覆盖去均值波形（不含 DC），且残差 |z|（符号无关）与 Fisher p
  仍略显著（残留少量确定结构），不宣称「相关完全消失」。详见
