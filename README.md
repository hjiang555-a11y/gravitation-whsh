...
```bash
# 权威综合差（推荐）：直接以专业提供的 30 秒「综合差」为总势差（不再自算分量）
gravitation-whsh \
  --professional-tidal data/professional_tidal_delta_30s.csv \
  --output results/wuhan_shanghai_full_tidal.csv \
  --plot results/wuhan_shanghai_full_tidal.svg

# 或仅替换海潮负荷、固体潮仍自算（旧口径，保留作对照）：
gravitation-whsh \
...
权威总势差的优先级从高到低：
`--professional-tidal`（专业综合差，直接作为总势差，分量列为空）>
`--professional-ocean`（专业海潮之差，覆盖海潮负荷）> `--harpos` > `--blq`。
HARPOS FES2014b 的海潮负荷幅度相对专业数据偏低约 3.9 倍（SHAO M₂ ~8 mm vs
专业 ~30 mm），故完整结果推荐用 `--professional-tidal`（或退而求其次的
`--professional-ocean`）。

仓库内置 `data/de440s.bsp`，默认即可完全离线运行；也可用
`--ephemeris /path/to/de440.bsp` 指定完整版 JPL 星历。日期范围按两个日期都
完整包含（`--professional-tidal` 模式下为 2026-06-20 至 09-10，共 83 天）。
程序同时生成 SVG 折线图；横轴为日期与时刻（`YYYY-MM-DD` / `HH:MM`，UTC），纵轴为
`SHAO − WUHN` 潮汐重力势差（m²/s²）。CSV 的 `elapsed_minutes` 列仍给出从起始时刻
算起的分钟数，便于与时间轴对应。

CSV 的关键列（`--professional-tidal` 模式下仅 `total` 与 `energy` 两列有值，
分量列为空）：

* `solid_effective_delta_m2_s2`：固体潮有效势差（综合差模式下为空）；
* `ocean_loading_delta_m2_s2`：海潮负荷随形势差（综合差模式下为空）；
* `total_tidal_delta_m2_s2`：总潮汐势差（综合差模式下 = 专业综合差插值）；
...
环外链路拍频（FXE_B8）及 17 组无跳点实验记录比对的分析（时间见
`clock/潮汐修正后的比值计算.pdf`），完整报告见 [clock/ANALYSIS.md](clock/ANALYSIS.md)。
...
- **主计算链已固化为专业综合差**：新增 `--professional-tidal` 入口，直接以专业
  30 秒综合差为权威总势差（不再自算固体潮+海潮拼接），向下游钟比对分析统一数据源。
